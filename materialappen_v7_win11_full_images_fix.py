...
def script_dir() -> Path: return Path(__file__).resolve().parent
def assets_path() -> Path:
    p = script_dir() / ASSETS_DIR; p.mkdir(parents=True, exist_ok=True); return p
def icon_path() -> Path: return assets_path() / APP_ICON
def product_img_dir() -> Path:
    p = script_dir() / PRODUCT_IMG_DIR; p.mkdir(parents=True, exist_ok=True); return p
def app_icon() -> QIcon:
...
def find_ui_file() -> Optional[Path]:
    for p in [script_dir() / "ui" / "main.ui", Path.cwd() / "ui" / "main.ui"]:
